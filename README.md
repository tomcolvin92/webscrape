...
- Article 6.2 cooperative approaches from the [CARP cooperative approaches page](https://unfccc.int/process-and-meetings/the-paris-agreement/article-6/article-62/carp/cooperative-approaches), including one row per approach and a selected most-updated report URL.
...
Run the cooperative approaches scraper to print one line per approach with the most updated report URL:

```bash
python scrape_cooperative_approaches.py
```

To save the extracted rows to CSV:

```bash
python scrape_cooperative_approaches.py --csv cooperative_approaches.csv
```
